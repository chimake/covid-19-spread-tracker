...
am4core.ready(function() {
	...
	var lastDate = new Date(covid_ng_total_timeline[covid_ng_total_timeline.length - 1].date);
	...
	var countryIndexMap = {};
	var list = covid_ng_timeline[covid_ng_timeline.length - 1].list;
    ...
    mapChart.geodataSource.url = "https://www.amcharts.com/lib/4/geodata/json/nigeriaHigh.json";
	...
	lineChart.data = JSON.parse(JSON.stringify(covid_ng_total_timeline));
  ...
  dateAxis.renderer.labels.template.adapter.add("fillOpacity", function(fillOpacity, target){
	...
	function showWorld() {
		...
		for (var i = 0; i < lineChart.data.length; i++) {
			var di = covid_ng_total_timeline[i];
			var dataContext = lineChart.data[i];
	...
	function updateTotals(index) {
		if (!isNaN(index)) {
			var di = covid_ng_total_timeline[index];
